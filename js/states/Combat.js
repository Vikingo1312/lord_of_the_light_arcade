        ...
        if (isSpecialInput) {
            if (this.p1.state === 'IDLE' || this.p1.state === 'WALK') {
                ...
                if (this.p1.specialEnergy >= 100 && !this.projectiles.some(p => p.owner === 'p1' && p.alive)) {
                    ...
                    // ─── SUPREME KEANO: DUAL PROJECTILES ───
                    if (this.p1.data.id === 'SupremeKeano') {
                        const img2 = this.game.assetManager.images['SupremeKeano_Cutin2'] || img;
    ...
    checkCombatCollisions() {
        ...
        if (this.p1.activeHitbox && p2CanBeHit) {
            ...
            if (Hitbox.checkCollision(hitR, hurtR)) {
                ...
                // Supreme Keano: 2x energy regen bonus
                if (this.p1.data.id === 'SupremeKeano') {
                    this.p1.specialEnergy = Math.min(this.p1.maxSpecialEnergy, this.p1.specialEnergy + (result === 'BLOCKED' ? 5 : 15));
                }
    ...
    preloadFighterSprites(char) {
        ...
        for (const suf of suffixes) {
            ...
            if (!this.game.assetManager.images[key]) {
                // Try the generic, widely-used un-prefixed version first to avoid 404 spam
                this.game.assetManager.queueImage(key, `assets/CHARACTERS/${char.folder}/${suf}.png`);
                // Fallback to the prefixed version if the generic one fails
                this.game.assetManager.queueImage(key + '_fallback', `assets/CHARACTERS/${char.folder}/${pfx}${suf}.png`);
            }
        }

        // ─── SUPREME KEANO: Preload extra animation frames ───
        if (char.id === 'SupremeKeano') {
            const extraFrames = ['keano_dive_kick', 'keano_low_kick', 'keano_hook_punch'];
            for (const frame of extraFrames) {
                const key = `SupremeKeano_${frame}`;
                if (!this.game.assetManager.images[key]) {
                    this.game.assetManager.queueImage(key, `assets/CHARACTERS/${char.folder}/${frame}.png`);
                }
            }
            if (!this.game.assetManager.images['SupremeKeano_Cutin2']) {
                this.game.assetManager.queueImage('SupremeKeano_Cutin2', 'assets/fx/char_projectiles/proj_supreme_keano_2.png');
            }
        }
